...
- **Add:** create `frontend/src/icons/<key>.svg` (camelCase key, e.g. `shoppingBag`) in the existing Feather style — `viewBox="0 0 24 24"`, `fill="none"`, `stroke="#808080"`, `stroke-width="2"`. Give the `stroke`/`fill` a visible placeholder color (use `#808080` for consistency) so the icon renders when opened standalone; the loader rewrites any hardcoded `stroke`/`fill` color to `currentColor` at load time so it inherits the theme color, so the exact value is not load-bearing (`fill="none"` is left intact). Then add `<key>: "Label"` to `iconLabels` in `frontend/src/icons.js` (position = picker order). Restart the dev server — the glob is resolved at build time.
