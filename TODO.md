...
- [ ] **Add `response.ok` checks to replication fetch calls** — Pull and push handlers in `replication.js` don't validate HTTP status before parsing JSON. Server errors (5xx) are silently ignored, making sync appear to work when it doesn't.
