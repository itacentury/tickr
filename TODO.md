...
- [x] **Externalize configuration via environment variables** — Key values are hardcoded with no env var override: database path (`data/tickr.db`), rate limit settings (100 req/60s), max SSE connections (10), SSE heartbeat interval (15s), server host/port. Use `os.getenv()` with sensible defaults.
