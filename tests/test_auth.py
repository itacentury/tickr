...
def test_login_success_sets_cookie(auth_enabled):
...
def test_login_wrong_password_returns_401(auth_enabled):
    client = TestClient(app, raise_server_exceptions=False)
...
def test_me_reports_true_when_authenticated(authed_client):
    resp = authed_client.get("/api/v1/auth/me")
    assert resp.status_code == 200
    assert resp.json() == {"authed": True, "enabled": True}


def test_me_reports_false_without_session(auth_enabled):
    client = TestClient(app, raise_server_exceptions=False)
...
def test_me_reports_enabled_false_when_auth_disabled(client):
